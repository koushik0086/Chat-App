...
export default function MessageBubble({ msg }) {
  ...
  // ─── Optimistic delete: remove from local store immediately, then emit socket ───
  const handleDelete = () => {
    if (!window.confirm('Delete this message?')) return
    // Optimistically remove from UI right away (no refresh needed)
    removeMessage(msg.roomId, msg._id)
    // Emit socket event so other users see it disappear too
    const socket = getSocket()
    if (socket) {
      socket.emit('delete-message', { messageId: msg._id, roomId: msg.roomId })
    }
  }
        ...
        <div className={`flex items-end gap-1.5 ${isOwn ? 'flex-row-reverse' : ''}`}>
          ...
          {canDelete && (
            <button
              ...
              className="opacity-0 group-hover:opacity-100 transition-opacity w-6 h-6 rounded-full bg-red-50 flex items-center justify-center text-red-400 hover:bg-red-100 flex-shrink-0 mb-1"
