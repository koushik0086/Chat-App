...
import { LogOut, Search, Paperclip, Send, MoreHorizontal } from 'lucide-react'
    ...
    <div className="flex h-screen overflow-hidden">
      ...
      <div className="w-64 flex-shrink-0 flex flex-col" style={{background:'#0f172a'}}>
        ...
        <div className="px-3 py-3 border-t border-white/10 flex items-center gap-2.5">
          <div className="w-8 h-8 rounded-full flex items-center justify-center text-xs font-medium flex-shrink-0" style={{background:'#eef2ff',color:'#6366f1'}}>
        ...
        {activeRoom ? (
          <>
            ...
            <div className="px-5 py-3.5 bg-white border-b border-gray-100 flex items-center gap-3">
              <div className="w-9 h-9 rounded-xl flex items-center justify-center font-medium text-sm flex-shrink-0" style={{background:'#eef2ff',color:'#6366f1'}}>#</div>
              <div>
                <p className="font-medium text-slate-800 text-sm"># {activeRoom.name}</p>
                <p className="text-slate-400 text-xs">12 members</p>
              </div>
            ...
            <div className="flex-1 overflow-y-auto px-5 py-4 flex flex-col gap-1">
              ...
              {roomMessages.map(msg => <MessageBubble key={msg._id} msg={msg}/>)}
              <div ref={bottomRef}/>
            </div>
            {/* Input */}
